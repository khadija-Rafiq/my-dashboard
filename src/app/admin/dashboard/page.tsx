...
//         {/* Navbar */}
//         <nav className=" text-white bg-red-600  p-4 shadow-lg flex justify-between">
  ...
  return (
    <ProtectedRoute>
      <div className="flex flex-col h-screen bg-gray-50">
        <nav className="bg-gradient-to-r from-red-500 to-red-700 text-white p-4 shadow-lg flex justify-between items-center rounded-b-lg">
          <h2 className="text-3xl font-semibold tracking-wide">Admin Dashboard</h2>
          <div className="flex space-x-2">
            {["All", "pending", "dispatch", "success"].map((status) => (
              <button
                key={status}
                className={`px-5 py-2 rounded-full transition-all shadow-md text-sm font-semibold focus:outline-none ${
                  filter === status ? "bg-white text-red-600" : "bg-red-600 hover:bg-red-800"
                }`}
                onClick={() => setFilter(status)}
              >
                {status.charAt(0).toUpperCase() + status.slice(1)}
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          <h2 className="text-2xl font-bold mb-4 text-center text-gray-800">Orders</h2>
                    ...
                    <tr className="cursor-pointer hover:bg-gray-100 transition-all" onClick={() => toggleOrderDetails(order._id)}>
                      ...
                      <td>
                        <select
                          value={order.status || ""}
                          onChange={(e) => handleStatusChange(order._id, e.target.value)}
                          className="bg-gray-100 p-2 rounded border focus:outline-none"
                        >
                          <option value="pending">Pending</option>
                          <option value="dispatch">Dispatch</option>
                          <option value="success">Completed</option>
                      ...
                      <td>
                        <button
                          ...
                          className="bg-red-500 text-white px-4 py-1 rounded-full hover:bg-red-700 transition"
                        >
                          Delete
                        </button>
                      </td>
